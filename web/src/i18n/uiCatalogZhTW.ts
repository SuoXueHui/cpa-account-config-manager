...
export const zhTWUI: Record<UIMessageKey, string> = {
  ...
  "ui.credit_usage_history_boundary_description": "只計費啟用後新採集的成功請求；既有 Token 彙總、額度視窗、巡檢和透支用量均保持不變。",
  ...
  "ui.adaptive_weekly_overdraft": "Codex 自適應最大透支",
  "ui.adaptive_weekly_overdraft_description": "按帳號記錄已驗證策略，僅在七日額度達到 99% 後啟用，並按 S1、S2、S4 有界升級。",
  "ui.adaptive_weekly_overdraft_request_behavior": "僅處理 schema v2 已選中帳號的 Codex 請求；低於 99%、用量過期或無法歸屬帳號時原樣放行。",
  "ui.adaptive_weekly_overdraft_automation_behavior": "明確的每週額度失敗才從 S1 升級到 S2、S4；瞬時限流不升級，401/402 立即停止探測。",
  "ui.adaptive_weekly_overdraft_availability_notice": "不保證固定透支量；策略受上游帳號和工作區狀態限制。",
  "ui.adaptive_weekly_overdraft_host_schema_required": "需要 CPA 請求生命週期 schema v2；目前主機不支援此模式。",
  "ui.adaptive_weekly_overdraft_configuration_warning": "啟動設定同時開啟了兩種透支模式，已保留作者模式並關閉自適應模式。",
	"ui.adaptive_token_first_controls": "Token-first 自適應控制",
	"ui.adaptive_token_drain": "99% 後工作階段集中調度",
	"ui.adaptive_token_drain_description": "將灰度工作階段在 30 分鐘內綁定至已驗證的破限憑證，同時限制每個憑證承載的工作階段數量。",
	"ui.adaptive_token_drain_percent": "調度灰度比例",
	"ui.adaptive_token_drain_max_sessions": "每憑證最大工作階段數",
	"ui.adaptive_tool_output": "工具輸出尾端相容",
	"ui.adaptive_tool_output_description": "使用獨立灰度允許 function_call_output 與 custom_tool_call_output 尾端請求參與注入，不再僅限使用者訊息尾端。",
	"ui.adaptive_tool_output_percent": "工具輸出灰度比例",
	"ui.sessions": "工作階段",
  "ui.test_original_overdraft_mode": "測試作者透支模式",
  "ui.test_adaptive_overdraft_mode": "測試自適應最大透支",
  "ui.adaptive_model_test_description": "自適應測試會按 S1、S2、S4 執行有界探測，並在明確的每週額度失敗後升級；不會同時載入作者模式。",
  "ui.adaptive_overdraft_strategy": "Adaptive {strategy}",
  "ui.adaptive_overdraft_armed": "Adaptive 待驗證",
  "ui.adaptive_overdraft_exhausted": "Adaptive 策略已耗盡",
  "ui.adaptive_overdraft_success_count": "觀測窗口成功 {count}",
  "ui.adaptive_overdraft_token_count": "{count} tok",
  "ui.adaptive_overdraft_injection_stats": "注入 {strategy}：{attempts} 次 / 成功 {successes} / 失敗 {failures}",
  "ui.adaptive_overdraft_workspace_stopped": "工作區已停止",
  "ui.adaptive_overdraft_authentication_stopped": "驗證已停止",
  "ui.adaptive_overdraft_stopped": "自適應透支已停止",
  "ui.adaptive_overdraft_no_further_probes": " · 不再探測",
