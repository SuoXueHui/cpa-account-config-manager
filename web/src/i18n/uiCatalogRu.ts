...
export const ruUI: Record<UIMessageKey, string> = {
  ...
  "ui.credit_usage_history_boundary_description": "Учитываются только успешные запросы после включения эксперимента. Существующие итоги токенов, окна квот, проверки и перерасход не меняются.",
  ...
  "ui.adaptive_weekly_overdraft": "Адаптивный максимальный перерасход Codex",
  "ui.adaptive_weekly_overdraft_description": "Сохраняет проверенную стратегию для каждого аккаунта, включается только после достижения 99% семидневной квоты и ограниченно повышает уровень S1, S2, S4.",
  "ui.adaptive_weekly_overdraft_request_behavior": "Обрабатываются только запросы Codex с выбранным аккаунтом в lifecycle schema v2. Запросы ниже 99%, с устаревшими данными или без привязки к аккаунту передаются без изменений.",
  "ui.adaptive_weekly_overdraft_automation_behavior": "Переход от S1 к S2 и S4 выполняется только при подтверждённой ошибке недельной квоты. Временное ограничение частоты не повышает уровень, а 401/402 немедленно останавливает проверку.",
  "ui.adaptive_weekly_overdraft_availability_notice": "Фиксированный объём перерасхода не гарантируется; результат зависит от состояния аккаунта и рабочей области upstream.",
  "ui.adaptive_weekly_overdraft_host_schema_required": "Требуется CPA request lifecycle schema v2; этот хост не поддерживает данный режим.",
  "ui.adaptive_weekly_overdraft_configuration_warning": "При запуске были включены оба режима перерасхода; авторский режим сохранён, адаптивный отключён.",
	"ui.adaptive_token_first_controls": "Адаптивные настройки token-first",
	"ui.adaptive_token_drain": "Концентрация сессий после 99%",
	"ui.adaptive_token_drain_description": "Закрепляет тестовые сессии за проверенными учётными данными на 30 минут и ограничивает число сессий на одну учётную запись.",
	"ui.adaptive_token_drain_percent": "Процент тестового планировщика",
	"ui.adaptive_token_drain_max_sessions": "Максимум сессий на учётные данные",
	"ui.adaptive_tool_output": "Совместимость хвоста вывода инструментов",
	"ui.adaptive_tool_output_description": "Отдельный тест разрешает инъекцию после function_call_output и custom_tool_call_output, а не только после пользовательского сообщения.",
	"ui.adaptive_tool_output_percent": "Процент теста вывода инструментов",
	"ui.sessions": "сессий",
  "ui.test_original_overdraft_mode": "Проверить авторский режим перерасхода",
  "ui.test_adaptive_overdraft_mode": "Проверить адаптивный максимальный перерасход",
  "ui.adaptive_model_test_description": "Адаптивная проверка ограниченно выполняет S1, S2 и S4 и повышает уровень только после подтверждённой ошибки недельной квоты. Авторский режим одновременно не загружается.",
  "ui.adaptive_overdraft_strategy": "Adaptive {strategy}",
  "ui.adaptive_overdraft_armed": "Adaptive ожидает проверки",
  "ui.adaptive_overdraft_exhausted": "Стратегии Adaptive исчерпаны",
  "ui.adaptive_overdraft_success_count": "Успешно в окне: {count}",
  "ui.adaptive_overdraft_token_count": "{count} tok",
  "ui.adaptive_overdraft_injection_stats": "Инъекция {strategy}: попыток {attempts} / успехов {successes} / ошибок {failures}",
  "ui.adaptive_overdraft_workspace_stopped": "Рабочая область остановлена",
  "ui.adaptive_overdraft_authentication_stopped": "Аутентификация остановлена",
  "ui.adaptive_overdraft_stopped": "Адаптивный перерасход остановлен",
  "ui.adaptive_overdraft_no_further_probes": " · дальнейших проверок не будет",
